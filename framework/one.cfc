...
*/
    ...
    private void function restoreFlashContext() {
    	//structAppend( request.context,{restoring="flash",doug=session.doug});
        if ( variables.framework.maxNumContextsPreserved > 1 ) {
        ...
        //structappend(request.context,{sessionkeyval=preserveKeySessionKey});
        try {
            if ( structKeyExists( session, preserveKeySessionKey ) ) {
            	structappend(request.context,{sessionkeyexists=true});
            	...
            	//structappend(request.context,{sessionkeyexists=false});
            }
        } catch ( any e ) {
            ...
            if ( keys == 'all' ) {
                ...
                for ( key in keyNames ) {
                    ...
                    if ( structKeyExists( request.context, key ) ) {
                        session[ preserveKeySessionKey ][ key ] = duplicate(request.context[ key ]);
                    }
                }
            }
        } catch ( any ex ) {
